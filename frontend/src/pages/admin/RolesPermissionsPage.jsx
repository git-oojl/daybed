...
function IconUserCog() {
  return (
    <svg width="20" height="20" viewBox="0 0 24 24" fill="none" aria-hidden="true">
      ...
      <path d="M19 10v4M21 12h-4" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round"/>
    </svg>
  );
}
...
const PERMISSION_GROUPS = [
  {
    ...
    label: "Dashboard",
    permissions: [
      { id: "dashboard_view", label: "Ver dashboard", admin: true, employee: false, editor: true, guest: false },
      { id: "dashboard_edit", label: "Editar widgets", admin: true, employee: false, editor: false, guest: false },
    ],
  },
  {
    id: "usuarios",
    label: "Usuarios",
    permissions: [
      { id: "users_view", label: "Ver usuarios", admin: true, employee: false, editor: true, guest: false },
      { id: "users_create", label: "Crear usuarios", admin: true, employee: false, editor: false, guest: false },
      { id: "users_edit", label: "Editar usuarios", admin: true, employee: false, editor: false, guest: false },
      { id: "users_delete", label: "Eliminar usuarios", admin: true, employee: false, editor: false, guest: false },
    ],
  },
  {
    id: "productos",
    label: "Productos",
    permissions: [
      { id: "products_view", label: "Ver productos", admin: true, employee: true, editor: true, guest: true },
      { id: "products_create", label: "Crear productos", admin: true, employee: false, editor: true, guest: false },
      { id: "products_edit", label: "Editar productos", admin: true, employee: false, editor: true, guest: false },
      { id: "products_delete", label: "Eliminar productos", admin: true, employee: false, editor: false, guest: false },
    ],
  },
  {
    id: "ventas",
    label: "Ventas",
    permissions: [
      { id: "sales_view", label: "Ver ventas", admin: true, employee: true, editor: true, guest: false },
      { id: "sales_export", label: "Exportar ventas", admin: true, employee: false, editor: false, guest: false },
    ],
  },
  {
    id: "configuracion",
    label: "Configuración",
    permissions: [
      { id: "settings_view", label: "Ver configuración", admin: true, employee: false, editor: false, guest: false },
      { id: "settings_edit", label: "Editar configuración", admin: true, employee: false, editor: false, guest: false },
    ],
  },
  {
    id: "reportes",
    label: "Reportes",
    permissions: [
      { id: "reports_view", label: "Ver reportes", admin: true, employee: false, editor: true, guest: false },
      { id: "reports_generate", label: "Generar reportes", admin: true, employee: false, editor: false, guest: false },
    ],
  },
];
  ...
  return (
    <div className="roles-permissions">
        ...
        <div className="roles-permissions-hero__overlay">
          <div className="roles-permissions-hero__content">
            ...
            <div className="roles-permissions-hero__text">
              <h1 className="roles-permissions-hero__title">Roles y permisos</h1>
              <p className="roles-permissions-hero__subtitle">
                Gestiona la asignación de roles y el control de acceso para empleados y administradores
              </p>
            </div>
          </div>
        </div>
      ...
      <div className="roles-permissions__content">
        ...
        {/* ===== ASIGNACIÓN DE ROLES ===== */}
        <section className="roles-permissions__section" aria-labelledby="asignacion-roles">
          <h2 id="asignacion-roles" className="roles-permissions__section-title">
            <IconUserCog />
            Asignación de roles
          </h2>
          <p className="roles-permissions__section-desc">
            Selecciona un rol para gestionar sus permisos y asignarlo a los usuarios
          </p>
          ...
          <div className="roles-permissions__roles-grid">
            {ROLES.map((role) => (
              ...
              >
                <div className="roles-permissions__role-icon" style={{ color: role.color }}>
                  {role.icon}
                </div>
                <div className="roles-permissions__role-info">
                  <h3 className="roles-permissions__role-name">{role.name}</h3>
                  <p className="roles-permissions__role-desc">{role.description}</p>
        ...
        {/* ===== CONTROL DE ACCESO ===== */}
        <section className="roles-permissions__section" aria-labelledby="control-acceso">
          <div className="roles-permissions__section-header">
            <div>
              <h2 id="control-acceso" className="roles-permissions__section-title">
                <IconLock />
                Control de acceso
              </h2>
              <p className="roles-permissions__section-desc">
                Gestiona los permisos para el rol <strong>{selectedRoleName}</strong>
              </p>
            </div>
              ...
              <table className="roles-permissions__table">
                ...
                <tbody>
                  {currentPermissions.map((perm, index) => {
                    const isFirstInGroup = index === 0 || currentPermissions[index - 1]?.groupId !== perm.groupId;
                    return (
                      <tr key={perm.id}>
                        ...
                        <td className="roles-permissions__table-status">
                          {editingPermissions ? (
                            ...
                            <span className={`roles-permissions__status-badge ${perm.hasPermission ? "roles-permissions__status-badge--active" : "roles-permissions__status-badge--inactive"}`}>
