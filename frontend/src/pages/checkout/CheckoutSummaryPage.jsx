        ...
        <div className="checkout-v3__layout">
          <div className="checkout-v3__flow">
            ...
            <section className="checkout-section-card">
              ...
              {!addressReadyForLookup || !postalCodeLooksValid ? <p className="checkout-field-hint">Para ubicarla en el mapa basta con calle y número, entidad, y al menos ciudad o código postal válido. Para finalizar el pedido sí pedimos la dirección completa.</p> : <p className="checkout-field-hint">Si encontramos una coincidencia clara, verás el mapa y la estimación de entrega aquí mismo.</p>}

              {addressError ? <div className="checkout-local-error" role="alert"><strong>{addressError.kind === API_ERROR_KINDS.EXTERNAL_SERVICE ? "No pudimos validar la ubicación ahora" : "Revisa la dirección"}</strong><p>{addressError.message}</p><p>Corrige los datos o inténtalo de nuevo. Si quieres, puedes terminar el pedido con la dirección escrita.</p>{selectedCandidate ? <button type="button" onClick={() => calculateEstimate(selectedCandidate)}>Recalcular entrega</button> : null}</div> : null}
