...
export default function ProductsPage() {
  const [products, setProducts] = useState([
    { id: 1, name: "Sofá Esquinero", price: 8999, stock: 5, category: "Sofás", status: "Activo", icon: "FaCouch" },
    { id: 2, name: "Mesa de Centro", price: 2499, stock: 8, category: "Mesas", status: "Activo", icon: "FaTable" },
    { id: 3, name: "Silla Ejecutiva", price: 3499, stock: 3, category: "Sillas", status: "Activo", icon: "FaChair" },
    { id: 4, name: "Estante Libros", price: 5999, stock: 0, category: "Almacenamiento", status: "Inactivo", icon: "FaBox" },
    { id: 5, name: "Lámpara de Pie", price: 1899, stock: 2, category: "Iluminación", status: "Activo", icon: "FaLightbulb" },
    { id: 6, name: "Sillón Reclinable", price: 12499, stock: 4, category: "Sofás", status: "Activo", icon: "FaCouch" },
  ]);
  ...
  const categories = ["Todas", "Sofás", "Mesas", "Sillas", "Almacenamiento", "Iluminación"];
    ...
    <div className="home-page dashboard-page">
      ...
      <main className="dashboard-container">
        <div className="dashboard-header-actions" style={{ display: 'flex', flexWrap: 'wrap', gap: '16px', justifyContent: 'space-between', alignItems: 'center', marginBottom: '24px' }}>
          <h2 style={{ fontSize: 'clamp(1.2rem, 2vw, 1.8rem)', color: '#6B4A2B' }}>Lista de productos</h2>
                  ...
                  {filteredProducts.length > 0 ? (
                    filteredProducts.map((product) => (
                      <tr key={product.id} style={{ borderBottom: '1px solid #F0EBE3' }}>
                        ...
                        <td style={{ padding: '12px 10px', fontWeight: 600, color: '#5C2E0B', fontSize: 'clamp(0.85rem, 1vw, 0.95rem)' }}>${product.price.toLocaleString('es-MX')}</td>
                        <td style={{ textAlign: 'center', padding: '12px 10px' }}>
                          <span className={`stock-badge ${product.stock === 0 ? 'stock-out' : ''}`} style={{
                            display: 'inline-block',
                            padding: '4px 14px',
                            borderRadius: '20px',
                            fontWeight: 600,
                            fontSize: 'clamp(0.75rem, 0.9vw, 0.85rem)',
                            background: product.stock === 0 ? '#FDECEA' : '#E8F5E9',
                            color: product.stock === 0 ? '#D32F2F' : '#2E7D32',
                          }}>
                            {product.stock}
                          </span>
                        </td>
                        <td style={{ textAlign: 'center', padding: '12px 10px' }}>
                          <button
                            ...
                            style={{
                              ...
                              cursor: 'pointer',
                              background: product.status === 'Activo' ? '#E8F5E9' : '#FDECEA',
                              color: product.status === 'Activo' ? '#2E7D32' : '#D32F2F',
                            }}
                          >
                            {product.status}
                          </button>
                        ...
                        <td style={{ textAlign: 'center', padding: '12px 10px' }}>
                          <div className="table-actions" style={{ display: 'flex', gap: '8px', justifyContent: 'center', flexWrap: 'wrap' }}>
                            <button onClick={() => handleOpenModal(product)} className="btn-edit" style={{
                              display: 'flex', alignItems: 'center', gap: '4px',
                              padding: '6px 14px', borderRadius: '6px', border: 'none',
                              background: '#8B5E3C', color: '#fff', fontSize: 'clamp(0.7rem, 0.85vw, 0.8rem)',
                              cursor: 'pointer', fontWeight: 500
                            }}>
                              <FaEdit size={12} /> Editar
                            </button>
                            <button onClick={() => handleDelete(product.id)} className="btn-delete" style={{
                              display: 'flex', alignItems: 'center', gap: '4px',
                              padding: '6px 14px', borderRadius: '6px', border: 'none',
                              background: '#D32F2F', color: '#fff', fontSize: 'clamp(0.7rem, 0.85vw, 0.8rem)',
                              cursor: 'pointer', fontWeight: 500
                            }}>
                    ...
                    <tr>
                      <td colSpan="6" style={{ textAlign: 'center', padding: '40px 20px', color: '#999' }}>
            ...
            <form onSubmit={handleSubmit}>
              ...
              <div className="form-group" style={{ marginBottom: '16px' }}>
                ...
                <select
                  ...
                  required
                  style={{ width: '100%', padding: '10px 14px', border: '1px solid #E8DCCC', borderRadius: '8px', fontSize: 'clamp(0.85rem, 1vw, 0.95rem)', background: '#FFFFFF' }}
                >
                  <option value="">Selecciona una categoría</option>
                  {["Sofás", "Mesas", "Sillas", "Almacenamiento", "Iluminación"].map((cat) => (
                    <option key={cat} value={cat}>{cat}</option>
                  ))}
                </select>
                {formData.category && (
                  <div style={{ marginTop: '8px', display: 'flex', alignItems: 'center', gap: '8px', padding: '8px 12px', background: '#F8F3ED', borderRadius: '8px' }}>
                    <span>Icono:</span>
                    {getCategoryIcon(formData.category)}
                    <span style={{ fontSize: '0.85rem', color: '#666' }}>{formData.category}</span>
                  </div>
                )}
              </div>
